...
trait CanHaveL1PrefetcherModule extends HasHellaCacheModule { this: RocketTileModuleImp =>
  ...
  val prefetchPort = prefetchOpt.map { pfu =>
    ...
    pfu.io.dmem.ready := dmem.req.ready
    dmem.req.valid := pfu.io.dmem.valid
    dmem.req.bits.addr := pfu.io.dmem.bits.addr
    dmem.req.bits.tag := DontCare
    dmem.req.bits.cmd := Mux(pfu.io.dmem.bits.write, M_PFW, M_PFR)
    ...
    dmem.keep_clock_enabled := pfu.io.dmem.valid
    dmem
  }
}
...
class L1PrefetcherIO(implicit p: Parameters) extends CoreBundle()(p) {
  val cpu = new Bundle {
    ...
    val miss = Input(Bool()) // Delayed 2 pipeline stages after request
  }
  val dmem = Decoupled(new L1PrefetchReq)
}
...
class ExampleL1Prefetcher(implicit p: Parameters) extends L1Prefetcher {
  ...
  when (s1_valid || io.dmem.ready) {
    miss_hold := false.B
  }

  io.dmem.valid := io.cpu.miss || miss_hold
  io.dmem.bits.addr := s2_req.addr
  io.dmem.bits.write := isWrite(s2_req.cmd)
}
